...
5. If it's *not* confident, or the document isn't actually an invoice (a statement, payment-info update, etc.), it skips auto-filing but still saves a copy into that project's **Statements & Others** subfolder and flags the row for a human to review — nothing gets silently misfiled or lost. If no project can be matched at all, the copy goes into a top-level **_Unmatched** folder instead.
...
Invoice Archive folders are created and Drive Folder IDs are populated (`createInvoiceArchiveFolders()` / `syncProjectReferenceFromDrive()` in `apps-script/DriveSetup.gs`, the latter on a daily trigger). One thing to keep in mind going forward:

- **Confirm the latest code is pasted into the live Apps Script project** after any update here (`Config.gs`, `Main.gs`, `Test.gs`, `DriveService.gs`, `GeminiService.gs`, `DriveSetup.gs`) — the code in this repo is the source of truth; the live editor needs to match it.
