...
// --- Nexus status sync -------------------------------------------------------------------------
// The coordinator uploads the latest Nexus export (CSV) and the dashboard MIRRORS each invoice's
// payment/lifecycle status onto our log, so Paid/Captured/Canceled reflect the system of record
// without anyone hand-editing rows (the whole reason we don't add manual "Rejected/Paid" labels).
// Matching is by invoice number (case-folded, trimmed). Nexus status -> our status:
//   PAID                                          -> Paid
//   POSTED / PENDING APPROVAL / IN PROGRESS / HOLD -> Captured  (still moving through the loop)
//   REJECTED / VOID                                -> Canceled  (terminal, never paid)
//   anything else                                  -> ignored
// Only "real invoice" rows are eligible to change (Filed/Captured/Paid/Canceled/Needs Review); a
// Duplicate row (points at the canon's file) and a Not an Invoice row are NEVER touched. The flow is
// preview-then-apply: previewNexusStatusUpdate summarizes what would change, applyNexusStatusUpdate
// commits it in resumable batches (each status change goes through updateInvoiceRow, so the file
// move + Review-Note stamp + Override-Log entry are identical to a manual edit).
...
/** Case-folded, trimmed invoice-number key for joining a Nexus row to a log row. Do NOT strip
 *  leading zeros or inner punctuation — invoice numbers like "AWJul23-503" are meaningful verbatim. */
function nexusInvoiceKey_(invoiceNumber) {
  return String(invoiceNumber == null ? '' : invoiceNumber).trim().toUpperCase();
}

/** Maps a raw Nexus status string to one of our statuses, or null if we don't act on it. */
...
/** Lifecycle "finality" rank — when one invoice number appears on several Nexus rows with different
 *  statuses, the most-final one wins (Paid over Canceled over Captured). */
...
/**
 * Parses the uploaded Nexus CSV into { map, stats }. `map` is invoiceKey -> {target, rawStatus},
 * de-duped so the most-final target wins per invoice number. Throws on an empty file or one missing
 * the "Number"/"Status" columns (so an accidental upload of the wrong file fails loudly).
 */
function parseNexusCsv_(csvText) {
  ...
  const numCol = header.indexOf('number');
  const statCol = header.indexOf('status');
  if (numCol === -1 || statCol === -1) {
  ...
  for (let i = 1; i < rows.length; i++) {
    const key = nexusInvoiceKey_(rows[i][numCol]);
    if (!key) { blank++; continue; }
    total++;
    const raw = rows[i][statCol];
    const target = mapNexusStatus_(raw);
    if (!target) { unknown++; continue; }
    const existing = map[key];
    if (!existing || nexusTargetRank_(target) > nexusTargetRank_(existing.target)) {
      map[key] = { target: target, rawStatus: String(raw == null ? '' : raw).trim() };
    }
  }
  return { map: map, stats: { total: total, blank: blank, unknown: unknown, uniqueMatched: Object.keys(map).length } };
}

/**
 * PREVIEW — reads the uploaded CSV and our log, and reports what an apply WOULD do, changing nothing.
 * Returns per-target counts, how many are already correct, how many log rows had no Nexus match, how
 * many matched but sit in an ineligible status, plus a small sample of the pending changes.
 */
function previewNexusStatusUpdate(csvText) {
  ...
  if (iNum === -1 || iStatus === -1) throw new Error('The Invoice Log is missing an Invoice Number or Status column.');

  let toPaid = 0, toCaptured = 0, toCanceled = 0, alreadyCorrect = 0, matchedIneligible = 0, logUnmatched = 0, changed = 0;
  const samples = [];
  for (let r = 1; r < values.length; r++) {
    const key = nexusInvoiceKey_(values[r][iNum]);
    if (!key) continue;
    const hit = map[key];
    if (!hit) { logUnmatched++; continue; }
    const cur = String(values[r][iStatus] || '').trim();
    if (!NEXUS_ELIGIBLE_STATUSES_[cur]) { matchedIneligible++; continue; }
    if (hit.target === cur) { alreadyCorrect++; continue; }
    changed++;
    if (hit.target === 'Paid') toPaid++;
    else if (hit.target === 'Captured') toCaptured++;
    else if (hit.target === 'Canceled') toCanceled++;
    if (samples.length < 15) {
      samples.push({
        invoiceNumber: String(values[r][iNum] || ''),
        vendor: iVendor > -1 ? String(values[r][iVendor] || '') : '',
        from: cur, to: hit.target, nexusStatus: hit.rawStatus
      });
    }
  }
  return {
    changed: changed, toPaid: toPaid, toCaptured: toCaptured, toCanceled: toCanceled,
    alreadyCorrect: alreadyCorrect, matchedIneligible: matchedIneligible, logUnmatched: logUnmatched,
    logRows: Math.max(0, values.length - 1), nexus: parsed.stats, samples: samples
  };
}

/**
 * APPLY (resumable) — walks the log from `startRow`, and for every eligible row whose Nexus target
 * differs from its current status, calls updateInvoiceRow to set it (which moves the file to the
 * right month bucket, stamps the Review Note, and records the Override Log entry). Stops before the
 * ~6-min kill and returns { done, nextRow, ... } so the client re-calls to continue. Idempotent: a
 * row already at its target is skipped, so re-running (or a resumed call) never double-applies. A
 * script lock prevents this from racing the automatic processInvoices run.
 *
 * @param {string} csvText  the same CSV the preview was run on (client keeps it and resends)
 * @param {number} startRow 1-based log-data row index to resume from (1 on the first call)
 */
function applyNexusStatusUpdate(csvText, startRow) {
  if (!canControlAutomation_()) throw new Error('You are not allowed to update invoice statuses.');
  ...
  try {
    const map = parseNexusCsv_(csvText).map;
    const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(CONFIG.SHEET_LOG_TAB);
    if (!sheet) throw new Error(`"${CONFIG.SHEET_LOG_TAB}" tab not found.`);
    const values = sheet.getDataRange().getValues();
    const header = values[0];
    ...
    // Read the Project Reference once for the whole batch — updateInvoiceRow accepts it cached so it
    // doesn't re-read per row (a row already carries its project; we only change status here).
    const referenceRows = getReferenceData_();
    const start = Math.max(1, Number(startRow) || 1);
    const startTime = Date.now();
    const MAX_RUN_MS = 2.5 * 60 * 1000;

    let processed = 0, changed = 0, nextRow = values.length, done = true;
    const errors = [];
    for (let r = start; r < values.length; r++) {
      if (Date.now() - startTime > MAX_RUN_MS) { nextRow = r; done = false; break; }
      processed++;
      const key = nexusInvoiceKey_(values[r][iNum]);
      if (!key) continue;
      const hit = map[key];
      if (!hit) continue;
      const cur = String(values[r][iStatus] || '').trim();
      if (!NEXUS_ELIGIBLE_STATUSES_[cur] || hit.target === cur) continue;
      const rowId = String(values[r][iRowId] || '').trim();
      if (!rowId) continue;
      try {
        updateInvoiceRow(rowId, { status: hit.target }, referenceRows);
        changed++;
      } catch (e) {
        errors.push({ invoiceNumber: String(values[r][iNum] || ''), message: e.message });
      }
    }
    return {
      done: done, nextRow: nextRow, processed: processed, changed: changed,
      totalRows: Math.max(0, values.length - 1), errors: errors
    };
  } finally {
    lock.releaseLock();
  }
