 ...
 *
 * Two sources, merged (deduped, order preserved):
 *   1. SEED_EXTRACTION_NOTES below — version-controlled, active on deploy.
 *   2. The optional "AI Notes" sheet tab (one note per row in a "Note" column) — lets the team add
 *      hints WITHOUT a code deploy. A note that stops being true can be deleted there or here.
 *
 * Keep each note short, factual, and always-true; this text is read on every extraction, so it
 * shapes every match. When a misread teaches a durable lesson, add it here in the same PR as the fix
 * (see CLAUDE.md's maintenance rule).
 */
...
/**
 * All active extraction notes: the "AI Notes" tab rows (if the tab exists) followed by the code
 * seed, deduplicated case-insensitively. Returns an array of strings; [] disables the section.
 */
function getExtractionNotes_() {
  ...
  SEED_EXTRACTION_NOTES.forEach(add);
