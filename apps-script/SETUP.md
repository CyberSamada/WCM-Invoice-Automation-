...
5. **After any future change** to `DashboardServer.gs` or `Dashboard.html`: if you've set up the GitHub Action (section 8 below), pushing to `main` bumps the live deployment automatically — nothing to do here. Otherwise, do it manually: Deploy > **Manage deployments** > pencil/edit icon on the existing deployment > Version: **New version** > Deploy. The shared URL stays the same either way; this just pushes the update live. (A brand-new deployment would give employees a different URL to re-bookmark — avoid that unless you actually want to retire the old one.)
...
**Keeping the live web app in sync:** the dashboard web app serves the code from its *deployed version*, not just the latest saved code — a plain `clasp push` alone won't update what the shared `/exec` URL shows. The GitHub Action handles this: after pushing code, it also runs `clasp deploy --deploymentId ...` against the actual production deployment (not `@HEAD`, which is never the right one), bumping it to a new version automatically. If you ever recreate the deployment (e.g. after deleting and remaking it), get the new ID with `clasp deployments` and update `--deploymentId` in `.github/workflows/deploy-apps-script.yml` to match — otherwise the Action keeps redeploying to an ID that no longer exists. Tip while iterating locally: the **Test deployment** URL (`/dev` instead of `/exec`, under Deploy > Test deployments) always serves the latest saved code with no version bumping, so you can preview changes there before they go live.
