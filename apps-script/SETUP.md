...
5. **After any future change** to `DashboardServer.gs` or `Dashboard.html`: bump the version manually — Deploy > **Manage deployments** > pencil/edit icon on the existing deployment > confirm **Web app** / **Execute as: Me** / the access setting are still correct > Version: **New version** > Deploy. The shared URL stays the same; this just pushes the update live. (A brand-new deployment would give employees a different URL to re-bookmark — avoid that unless you actually want to retire the old one. Also avoid `clasp deploy` for this step — see section 8's note on why.)
...
**One thing `clasp push` does not do — and one thing to deliberately avoid automating:** the dashboard web app serves the code from its *deployed version*, not just the latest saved code, so a plain `clasp push` alone won't update what the shared `/exec` URL shows. The obvious fix would be having the GitHub Action also run `clasp deploy --deploymentId <id>` to bump that deployment automatically — this was tried, and it's a trap: that command calls the Apps Script API's `deployments.update` under the hood, which does not reliably preserve the deployment's "Web app" configuration (execute-as, who-has-access, entry point). It can silently downgrade the deployment to library-only, breaking the live URL, with no error to show for it. The Apps Script UI's own "Deploy" button doesn't have this problem — it always re-applies the full config. So: after `clasp push`, bump the version **manually** through the UI (step 5 above), and don't wire `clasp deploy` into any automation for this project. Tip while iterating locally: the **Test deployment** URL (`/dev` instead of `/exec`, under Deploy > Test deployments) always serves the latest saved code with no version bumping, so you can preview changes there before they go live.
