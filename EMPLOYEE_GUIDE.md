...
Below that, filters narrow the invoice list. The **Status** and **Project** filters are dropdowns where you can tick **several at once** (ticking a main project includes all its subprojects). You can also search by **vendor** or **invoice #**, set an **amount** range, and pick a **date range** — filtered by processed, received, or invoice date. A **Sort by** control reorders whatever the filters found (newest/oldest, by vendor, project, amount, or status). A **Show** control sets how many rows appear at once — **100 per page** (the default), 50, or All; with 100 or 50, Prev/Next buttons appear under the table. Note that ticking the header checkbox selects everything on the **current page**, not the whole filtered list — that's deliberate, so a bulk edit can never reach rows you haven't looked at. Your selection survives paging: tick rows on one page, move to the next, and the bulk bar keeps counting them and tells you how many are on other pages. Saving an edit leaves you on the page you were working on rather than sending you back to the first, and **Prev / Next** in the preview walks the whole filtered list, carrying the table along with it when it crosses onto the next page.
...
- 📄 **Preview** the filed PDF right on the page — with an edit panel beside it, the note shown above the panel, and **Prev / Next** buttons to work through a stack without closing it. You can **select and copy text straight off the invoice** in the preview — just highlight it, no button to press. Use the **− / + / Fit** buttons above the document to zoom (or Ctrl + scroll wheel). Once zoomed in, drag a **blank part of the page** to move it around, and drag **over text** to select it — same as Acrobat, with no mode to switch. Holding the **space bar** turns the whole page into a hand so you can drag from anywhere. The **file's location** is on a bar at the top with a **copy button** if you need the path. Scanned/image invoices have no text in them to select, so those show in the standard viewer and you'd read the values off manually.
